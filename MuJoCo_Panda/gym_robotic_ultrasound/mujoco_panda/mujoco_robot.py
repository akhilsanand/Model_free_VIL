...
class MujocoRobot(object):
    ...
    def __init__(self, model_path=None, render=True, config=None, prestep_callables={}, poststep_callables={}, from_model=False):
        ...
        print(self._nq,self.qpos_joints)
        self._all_joint_names = [
            self.model.joint_id2name(j) for j in range(self._nq)]#self.qpos_joints]
    ...
    def mass_matrix(self):
        ...
        mjp.cymj._mj_fullM(self._sim.model, mass_matrix, self._sim.data.qM)
        mass_matrix = np.reshape(mass_matrix, (len(self._sim.data.qvel), len(self._sim.data.qvel)))
        mass_matrix = mass_matrix[self.movable_joints, :][:, self.movable_joints]
        return mass_matrix