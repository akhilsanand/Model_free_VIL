    ...
    def __init__(self, position_as_action=False, controller="VIC", reward_type="dense", pert_type='none' \
        ...
        self.robot = PandaArm(model_path=MODEL_PATH + 'panda_touch.xml',
                             render=False, compensate_gravity=True, \
                              grav_comp_model_path=MODEL_PATH + 'franka_panda_soft_touch.xml',\
        ...
        self.init_jpos = np.array([-3.84e-03, -8.69e-01,  2.27e-03, -2.37e+00 , 1.49e-03 , 1.50e+00 , 8.23e-01])
...
#design teh
    ...
    def change_goal(self):
        if self.goal_type == "random":
            ...
            self.x_d[0:3] -= np.array([-0.0, -0.0, 0.02])
            self.fd = np.array([0,0,20,0,0,0])
            #self.x_d[]
    ...
    def step(self, action):
        ...
        if self.i == 20:
            ...
            self.fd = np.array([0, 0, 30, 0, 0, 0])
        x_dot = self.obs_dict['vel'][0:3].copy()
        self.action = action.copy()
        K_target = 3000 * action
        K_target[0], K_target[1] =1000,1000
        ...
        for k in range(100):
            ...
            self.controller.set_goal(K, self.x_d, self.goal_ori, 0 * self.x_d_dot, 0 * self.x_d_ddot, self.fd)
        ...
        print("cartesian force: ",self.obs_dict['cartesian_force'][2])
        print("sensed force: ",self.obs_dict['FT'][2])
...
def make_panda_touch_env(env_cfg):
